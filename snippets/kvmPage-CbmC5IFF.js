    ...
    a({
      ...
      setup(e) {
        const t = l(),
          ...
          pasteToRemoteThrottled = async (text, keymap, cps) => {
            const rate = cps || CHARS_PER_SECOND,
              chunkSize = Math.max(1, Math.floor(rate / 10)),
              intervalMs = Math.round((chunkSize / rate) * 1000);
            for (let i = 0; i < text.length; i += chunkSize) {
              const chunk = text.slice(i, i + chunkSize);
              await le.pasteToRemote(chunk, keymap);
              if (i + chunkSize < text.length)
                await new Promise((r) => setTimeout(r, intervalMs));
            }
          },
          setQueueFromClipboard = async () => {
            try {
              const txt = await navigator.clipboard.readText(),
                names = txt
                  ...
                  .filter(Boolean);
              if (!names.length) {
                Lt.error(R("main.patchQueueEmpty"));
                return;
              }
              q.value = names.map((name, i) => ({
                id: i,
                name,
          ...
          clearQueue = () => {
            ...
            nx.value = 0;
            try {
              sessionStorage.removeItem("glkvm-patch-queue");
            } catch (_) {}
          },
          pasteNext = async () => {
            var ref, res;
            let content = s.text && s.text.trim() ? s.text : "";
            if (!content)
              try {
            ...
            if (firstLine && !/^(diff --git|From\s)/.test(firstLine) && typeof Lt.warning === "function")
              Lt.warning(R("main.patchHeaderWarning"));
            if (q.value.length > 0) {
              const cur = q.value[nx.value];
              if (!cur) return;
              try {
                s.loading = !0;
                await pasteToRemoteThrottled(content, a.value, CHARS_PER_SECOND);
                const newNx = nx.value + 1;
                q.value = q.value.map((entry, i) => ({
                  ...entry,
                  status:
                    i < newNx ? "done" : i === newNx ? "current" : "pending",
                }));
                nx.value = newNx;
                s.text = "";
                if (newNx >= q.value.length)
                  null == (ref = t.value) || ref.handleClose();
              } catch (i) {
                416 === (null == (res = i.response) ? void 0 : res.status)
        ...
        try {
          const stored = sessionStorage.getItem("glkvm-patch-queue");
          if (stored) {
            const parsed = JSON.parse(stored);
            if (Array.isArray(parsed) && parsed.length) {
              q.value = parsed;
              const idx = parsed.findIndex((e) => e.status === "current");
              nx.value = idx >= 0 ? idx : 0;
            }
          }
        } catch (_) {}
        return (e, t) => {
          ...
          return (
            ...
            g(
              ...
              },
              {
                ...
                default: d(() => [
                  r("div", Wl, [
                    r("div", { class: "patch-queue-section" }, [
                      r("div", { class: "patch-queue-title" }, [
                        C(p(e.$t("main.patchQueue")), 1),
                      ]),
                      u(q).length > 0
                        ? (i(),
                          n(
                            "div",
                            { key: 0, class: "patch-queue-list" },
                            u(q).map((entry, i) =>
                              r(
                                "div",
                                {
                                  key: entry.id,
                                  class: "patch-queue-item " + entry.status,
                                },
                                [
                                  r("span", { class: "patch-queue-index" }, [
                                    C(p(String(i + 1))), 1,
                                  ]),
                                  r("span", { class: "patch-queue-name" }, [
                                    C(p(entry.name)), 1,
                                  ]),
                                  r("span", { class: "patch-queue-status" }, [
                                    C(p(e.$t("main.patchStatus" + entry.status.charAt(0).toUpperCase() + entry.status.slice(1))), 1),
                                  ]),
                                ]
                              )
                            )
                      ...
                      u(q).length > 0
                        ? (i(),
                          n("div", { key: 1, class: "patch-queue-progress" }, [
                            C(p(e.$t("main.patchQueueProgress").replace("{current}", String(u(nx) + 1)).replace("{total}", String(u(q).length))), 1),
                          ]))
                        : y("", !0),
                      r("div", { class: "patch-queue-actions" }, [
                        o(u(Pt), {
                          size: "small",
                          onClick: setQueueFromClipboard,
                        }, {
                          default: d(() => [C(p(e.$t("main.setQueueFromClipboard")), 1)]),
                          _: 1,
                        }, 8, ["onClick"]),
                        u(q).length > 0
                          ? (i(),
                            n(o(u(Pt), {
                              ...
                              default: d(() => [C(p(e.$t("main.clearQueue")), 1)]),
                              _: 1,
                            }, 8, ["onClick"]), 1))
                          : y("", !0),
                      ]),
                    ...
                    o(
                      ...
                      {
                        default: d(() => [C(p(e.$t("main.pasteNextPatch")), 1)]),
                        _: 1,
                      },
                      8,
                      ["loading"],
